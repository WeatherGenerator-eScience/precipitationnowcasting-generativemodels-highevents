...
from _model_builder_DGMR import DGMR
...
#---------- Run with wandb ----------------
run = wandb.init(
    project="high-precipitation-forecasting",
    config={
        "batch_size": batch_size,
        "epochs": N_EPOCHS,
        "lr_g": lr_g,
        "lr_d": lr_d,
        "l_adv": 1,  # taken from default value in DGMR class
        "l_rec": 0.01,  # taken from default value in DGMR class
        "g_cycles": g_cycles,
        "label_smoothing": 0,  # taken from default value in DGMR class
        "x_length": x_length,
        "y_length": y_length,
        "rnn_type": "GRU",
...
model = DGMR(
    ...
    g_cycles=config.g_cycles,
    label_smoothing=config.label_smoothing,
    l_adv=config.l_adv,
    l_rec=config.l_rec,
    norm_method=config.norm_method,
    downscale256=config.downscale256,
    rec_with_mae=config.rec_with_mae,
    balanced_loss=balanced_loss,
    extended_balanced=extended_balanced_loss,
    rmse_loss=rmse_loss,
    temp_data=temp_data,
)
